...
void counting_sort(key_value_pairs *pairs, unsigned binary_shift) {
    ...
    int byte_mask = initial_byte_mask << binary_shift;
...
void radix_sort(key_value_pairs *pairs) {
    for (int byte_shift = 0; byte_shift < sizeof(int); ++byte_shift) {
...
int main() {
    ...
    std::vector<key_value_pair> v_pair;
    ...
    while (scanf("%s\t%s", input_key, value) > 0) {
        sscanf(input_key, "%d.%d.%d", &day, &month, &year);
        ...
        key_value_pair kvp;
        kvp.key = year * 10000 + month * 100 + day;
        strcpy(kvp.input_key, input_key);
        strcpy(kvp.value, value);

        v_pair.emplace_back(kvp);

        pairs.data[pairs.count].key = kvp.key;
    ...
    auto duration_radix = std::chrono::duration_cast<std::chrono::milliseconds>(stop_radix - start_radix);
    std::cout << "Radix Sort Time: " << duration_radix.count() << " milliseconds" << '\n';

    auto start_std = std::chrono::high_resolution_clock::now();
    std::sort(v_pair.begin(), v_pair.end(), [](const key_value_pair& a, const key_value_pair& b) {
        return a.key < b.key;
    });
    auto stop_std = std::chrono::high_resolution_clock::now();
    auto duration_std = std::chrono::duration_cast<std::chrono::milliseconds>(stop_std - start_std);
    std::cout << "std::sort Time: " << duration_std.count() << " milliseconds" << '\n';


    for (uint64_t i = 0; i < pairs.count; ++i) {
        if (pairs.data[i].key == v_pair[i].key) {
            exit(1);
        }
